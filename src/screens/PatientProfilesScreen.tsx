  ...
  return (
    <SafeAreaView style={[styles.safeArea, isDarkMode && { backgroundColor: '#1F2937' }]}>
      ...
      <ScrollView contentContainerStyle={styles.container}>
        <View style={[styles.alertBox, isDarkMode && { backgroundColor: 'rgba(14, 165, 233, 0.2)' }]}>
          <Ionicons name="information-circle" size={20} color={isDarkMode ? '#38BDF8' : "#0284C7"} />
          <Text style={[styles.alertText, isDarkMode && { color: '#38BDF8' }]}>
            {t('manage_records_desc')}
          </Text>
        </View>

        <Text style={[styles.sectionTitle, isDarkMode && { color: '#F3F4F6' }]}>{t('records_list')} ({profiles.length})</Text>

        {profiles.map((profile) => (
          <TouchableOpacity 
            key={profile.id} 
            style={[styles.profileCard, SHADOWS.card, isDarkMode && { backgroundColor: '#374151' }]}
            activeOpacity={0.7}
            onPress={() => navigation.navigate('ProfileDetail', { profileId: profile.id })}
          >
            <View style={styles.cardHeader}>
              <View style={styles.cardHeaderLeft}>
                <Ionicons name="person-circle" size={40} color={isDarkMode ? '#818CF8' : COLORS.primary} style={{ marginRight: 10 }} />
                <View>
                  <Text style={[styles.profileName, isDarkMode && { color: '#F3F4F6' }]}>{profile.name}</Text>
                  <Text style={[styles.profileId, isDarkMode && { color: '#9CA3AF' }]}>{profile.patientId}</Text>
                </View>
              </View>
              <View style={[styles.badge, { backgroundColor: profile.relationship === 'Bản thân' ? (isDarkMode ? 'rgba(99, 102, 241, 0.2)' : '#EEF2FF') : (isDarkMode ? '#4B5563' : '#F3F4F6') }]}>
                <Text style={[styles.badgeText, { color: profile.relationship === 'Bản thân' ? (isDarkMode ? '#818CF8' : COLORS.primary) : (isDarkMode ? '#D1D5DB' : COLORS.placeholder) }]}>
        ...
        <TouchableOpacity style={[styles.addButton, SHADOWS.card, isDarkMode && { backgroundColor: '#374151', borderColor: '#4B5563' }]} activeOpacity={0.7} onPress={handleAddNewProfile}>
          <Ionicons name="add" size={24} color={isDarkMode ? '#818CF8' : COLORS.primary} />
          <Text style={[styles.addButtonText, isDarkMode && { color: '#818CF8' }]}>{t('add_new_record')}</Text>
        </TouchableOpacity>
